...
test_that("supp data that has a mismatch on a floating point level but not when visualzied as numbers still merges (#33)", {
  visitnum_raw_xpt <-
    as.raw(c(
      0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 
      0x01, 0x01, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 
      0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 
      0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 
      0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 
      0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00
    ))
  visitnum_raw_as_numeric <-
    as.raw(c(
      0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 
      0x01, 0x01, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 
      0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 
      0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 
      0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 
  ...
  visitnum_xpt <- packBits(visitnum_raw_xpt, type="double")
  visitnum_as_numeric <- packBits(visitnum_raw_as_numeric, type="double")
  # Both look like they are 24.04; visitnum_xpt came from loading a .xpt file;
  # visitnum_as_numeric came from as.numeric("24.04")
  expect_false(visitnum_xpt == visitnum_as_numeric)
  expect_true(visitnum_as_numeric == as.numeric("24.04"))
  dataset <-
    data.frame(
      STUDYID = "Study1",
      DOMAIN = "VE",
      USUBJID = "ID1",
      VISITNUM = visitnum_xpt
    )
  supp_data <-
    data.frame(
      STUDYID = "Study1",
      ...
      IDVARVAL = "24.04",
      QNAM = "VEEPCHGI",
      QLABEL = "Epi/Pandemic Related Change Indicator",
      QVAL = "N",
      QORIG = "CRF",
      QEVAL = ""
    )
  expect_s3_class(
    combine_supp(dataset=dataset, supp=supp_data),
    "data.frame"
  )
})
